"""
Trains the M13 follow-up real cat image classifier.

See docs/specs/M13-cat-disease-detection.md. Run from ml-service/ (venv active, torch/
torchvision installed):

    python -m training.cat_image_model_train
...
from app.models.cat_image_model import DISEASES
from app.models.image_model import PREPROCESS, build_model

DATA_PATH = Path(__file__).resolve().parents[1] / "data" / "cat-images"
MODEL_PATH = Path(__file__).resolve().parents[1] / "models" / "cat_image_model.pt"
...
def update_registry(metrics: dict, model_path: Path = MODEL_PATH, registry_path: Path = REGISTRY_PATH) -> None:
    entry = (
        f"| `{model_path.name}` | Flea Allergy / Healthy / Ringworm / Scabies "
        f"(replaces M13's URI/FIV research list — no image data for those) | "
        f"cat-images ({metrics['n_samples']} images) | "
